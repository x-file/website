...
<body>
	...
	<form:form id="inputForm" modelAttribute="video" action="${ctx}/cms/video/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="videoType" class="input-mini">
					<form:options items="${fns:getDictList('cms_video_type')}" itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea id="content" htmlEscape="true" path="videoData.content" rows="4" maxlength="200" class="input-xxlarge"/>
				<sys:ckeditor replace="content" uploadPath="/cms/video" />
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<script type="text/javascript">
					var videoSelect = [];
					function videoSelectAddOrDel(id,title){
						var isExtents = false, index = 0;
						for (var i=0; i<videoSelect.length; i++){
							if (videoSelect[i][0]==id){
								isExtents = true;
								index = i;
							}
						}
						if(isExtents){
							videoSelect.splice(index,1);
						}else{
							videoSelect.push([id,title]);
						}
						videoSelectRefresh();
					}
					function videoSelectRefresh(){
						$("#videoDataRelation").val("");
						$("#videoSelectList").children().remove();
						for (var i=0; i<videoSelect.length; i++){
							$("#videoSelectList").append("<li>"+videoSelect[i][1]+"&nbsp;&nbsp;<a href=\"javascript:\" onclick=\"videoSelectAddOrDel('"+videoSelect[i][0]+"','"+videoSelect[i][1]+"');\">×</a></li>");
							$("#videoDataRelation").val($("#videoDataRelation").val()+videoSelect[i][0]+",");
						}
					}
					$.getJSON("${ctx}/cms/video/findByIds",{ids:$("#videoDataRelation").val()},function(data){
						for (var i=0; i<data.length; i++){
							videoSelect.push([data[i][1],data[i][2]]);
						}
						videoSelectRefresh();
					});
					$("#relationButton").click(function(){
						top.$.jBox.open("iframe:${ctx}/cms/video/selectList?pageSize=8", "添加相关",$(top.document).width()-220,$(top.document).height()-180,{
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:radiobuttons path="videoData.allowComment" items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
			</div>
		</div>
		<div class="control-group">
